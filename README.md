...
> 輔仁大学 情報管理学科 第38期 卒業制作（2021年9月 – 2022年11月）<br>
...
資産が変動する**書き込み操作はすべて FastAPI 経由**とし、注文処理と残高計算をサーバー側に集約しています。
各リクエストは Firebase ID トークンを検証してから処理されます。

一方、口座残高・保有銘柄・未約定注文・取引履歴は、クライアントが **Firestore の `onSnapshot` でリアルタイムに監視**しています。
指値注文の約定（30 秒ごとのスケジューラー）やコピートレードによる自動発注など、
**ユーザー自身の操作によらずサーバー側で非同期に資産が変動する**ため、ポーリングや手動更新なしで
変更を即座に画面へ反映できるようにしました。API は Firestore に書き込むだけで、更新後の状態をレスポンスで返す必要がありません。

相場データは変更通知の仕組みがない外部 API のため、クライアントが CoinGecko API を画面ごとの間隔でポーリングしています
（銘柄詳細 4 秒・銘柄一覧 10 秒・ホーム 15 秒、画面離脱時にタイマーを解除）。バックエンドも約定判定用に同 API を 6 秒ごとに取得しています。
